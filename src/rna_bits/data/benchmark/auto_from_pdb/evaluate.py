...
def run_evaluate(DIR):
    ...
    PDB_DIR = get_path(DIR+"/generated/")
    print(DIR)
    asdf = os.path.join(DIR, "..", "provided_ss")
    PROVIDED_DIR = get_path(asdf)

    OUT_DIR = get_path(DIR+"/measures/", create=True)

    pdb_dirs = [a for a in os.listdir(PDB_DIR) if os.path.dirname(os.path.join(PDB_DIR,a))]
    print(PDB_DIR)
    print(pdb_dirs)
    assert(len(pdb_dirs))
    pdb_dirs.sort()
    ...
    for i, fn in enumerate(pdb_dirs):
        ...
        for ii, fnn in enumerate(struct_filenames):
            ...
            cons_index = os.path.join(PROVIDED_DIR, fn +".cons.index_noloose")

            ref = os.path.join(REF_PDB_DIR, fn + ".pdb")
            ref_index = os.path.join(PROVIDED_DIR, fn +".ref.index_noloose")
