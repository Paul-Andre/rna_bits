...
if "ROSETTA3" in os.environ:
    ...
    print(
        "$ROSETTA3 or $ROSETTA not in environment. Install Rosetta and set $ROSETTA to Rosetta's main/source/ directory."
    )
    exit(1)
...
for a in PAIRS:
    for b in PAIRS:
        ...
        shutil.copy(
            pjoin(IN_DIR, make_rosetta_name(a, b) + ".out"),
            pjoin(OUT_DIR, n, "silent.out"),
        )
