...
center = torch.tensor([250.,250.])

import random
def make_block():
    pos = torch.autograd.Variable(torch.normal(center, 20.), requires_grad=True)
    rot = torch.autograd.Variable(torch.tensor(3.), requires_grad=True)
    block = Block(pos, rot)
...
def make_glued_bead():
    pos = torch.autograd.Variable(torch.normal(center, 20.))
    bead = Bead(pos)
    beads.append(bead)
    return bead

def make_floating_bead():
    pos = torch.autograd.Variable(torch.normal(center, 50.), requires_grad=True)
    bead = Bead(pos)
    beads.append(bead)
    variables.append(pos)
    return bead
...
while running:
    ...
    for block in blocks:
        ...
        points = [apply_rotation(torch.tensor(p), block.rot)+block.pos for p in points]
        print(points[0])
    ...
    for chain in chains:
        ...
        for i in range(len(chain.beads)-1):
            a = chain.beads[i].pos
            b = chain.beads[i+1].pos
            pygame.draw.line(screen, (0,0,255), tensor_to_tuple(a), tensor_to_tuple(b), 2)
